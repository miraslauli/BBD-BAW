...
def get_sales_statistics(
    ...
    total_revenue = (
        orders_query.with_entities(func.sum(Order.total_amount)).scalar() or 0.0
    )
    ...
    revenue_by_category = (
        db.query(Category.name, func.sum(OrderItem.total_price).label("revenue"))
        .join(Product)
        .join(OrderItem)
        .join(Order)
        .filter(Order.created_at >= start_date)
    ...
    daily_sales_list = [
        {"date": str(date), "orders": orders, "revenue": float(revenue or 0)}
...
def get_user_statistics(
    ...
    total_orders = db.query(Order).count()
    avg_orders_per_user = (
        total_orders / users_with_orders if users_with_orders > 0 else 0.0
    )
...
def get_product_statistics(
    ...
    low_stock_threshold: int = Query(
        10, ge=1, description="Próg dla produktów o niskim stanie"
    ),
):
    """Statystyki produktów"""
    total_products = db.query(Product).count()
    ...
    most_popular_list = [
        {"name": name, "order_count": order_count, "total_sold": int(total_sold)}
        for name, order_count, total_sold in most_popular
    ]

    # Najmniej popularne produkty
    least_popular = (
        db.query(Product.name, Product.stock_quantity)
    ...
    least_popular_list = [
        {"name": name, "stock_quantity": stock_quantity, "order_count": 0}
