...
def initialize_database():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Błąd inicjalizacji bazy danych: {str(e)}")
...
def create_test_data():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Błąd tworzenia danych testowych: {str(e)}")
...
def create_database_backup(backup_name: Optional[str] = Query(None)):
    ...
    try:
        ...
        if backup_path:
            return {"message": "Kopia zapasowa została pomyślnie utworzona", "backup_path": backup_path}
        else:
            raise HTTPException(status_code=500, detail="Nie udało się utworzyć kopii zapasowej")
    except Exception as e:
...
def restore_database(backup_path: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail="Nie udało się przywrócić bazy danych")
    except Exception as e:
...
def create_database_dump(dump_name: Optional[str] = Query(None)):
    ...
    try:
        ...
        if dump_path:
            return {"message": "Zrzut SQL został utworzony pomyślnie", "dump_path": dump_path}
        else:
            raise HTTPException(status_code=500, detail="Nie udało się utworzyć zrzutu SQL")
    except Exception as e:
...
def get_tables_info():
    ...
    try:
        info = get_tables_and_indexes()
        return info
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Błąd pobierania informacji o tabelach: {str(e)}")
...
def perform_crud_operations():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Błąd wykonywania operacji CRUD: {str(e)}")
...
def test_database_indexes():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Błąd testowania indeksów: {str(e)}")
